...
void PrintTrain(TrainCar* train) {
  ...
  int total_weight,num_engines,num_freight_cars,num_passenger_cars,num_dining_cars,
  num_sleeping_cars;
  ...
  int total_cars = num_engines+num_freight_cars+num_passenger_cars+num_dining_cars
  +num_sleeping_cars;
  ...
  if (num_passenger_cars > 0) {
    ...
    if (dist_to_dining < 0) {
      ...
      std::cout << ", avg distance to dining = " << std::setprecision(1) << std::fixed
       << dist_to_dining;
    }
  }
...
void SimpleTrainTest() {
  ...
  assert (simple->next->next->next->next->next->getID() == simple->next->next->next->next
  ->getID()+1);
...
void ShipFreightHelper(MTRand_int32 &mtrand, int num_engines, int num_cars, int min_speed
, int max_cars_per_train) {
  ...
  std::vector<TrainCar*> trains = ShipFreight(all_engines, all_freight, min_speed,
   max_cars_per_train);
  ...
  for (unsigned int i = 0; i < trains.size(); i++) {
    ...
    int total_weight,num_engines,num_freight_cars,num_passenger_cars,num_dining_cars,
    num_sleeping_cars;
    TotalWeightAndCountCars(trains[i],total_weight,num_engines,num_freight_cars,
    num_passenger_cars,num_dining_cars,num_sleeping_cars);
    int total_cars = num_engines+num_freight_cars+num_passenger_cars+num_dining_cars+
    num_sleeping_cars;
...
void StudentTests() {
  ...
  //tests small number of alowwed cars and high speed
  MTRand_int32 mtrand_autoseed(time(NULL));
  for (int i = 1; i <= 5; i++) {
    std::cout << "============================================================================"
     << std::endl;
    std::cout << "SHIP FREIGHT TEST, RANDOM SEED #" << i << std::endl;
    ShipFreightHelper(mtrand_autoseed,10,25,100,6);
  }

  TrainCar* t;

  //for seprating train with lots of engines split up
  t = NULL;
  PushBack(t, TrainCar::MakeEngine()); 
  PushBack(t, TrainCar::MakePassengerCar());
  PushBack(t, TrainCar::MakePassengerCar());
  ...
  //for separating train with lots of engines close together
  t = NULL;
  PushBack(t, TrainCar::MakeEngine()); 
  PushBack(t, TrainCar::MakeEngine());
  PushBack(t, TrainCar::MakeEngine());
  ...
  //testing where passenger cars will be uneven and engines will be uneven
  t = NULL;
  PushBack(t, TrainCar::MakePassengerCar());
  PushBack(t, TrainCar::MakeEngine()); 
  PushBack(t, TrainCar::MakeEngine());
